...
def create_wordpress_post(title, content, image_id, category, quality):
    post_data = {
...
async def new_post(client, message):
    try:
        ...
        imdb_response = (await client.listen(message.chat.id)).text.strip()
        imdb_id_match = re.search(r'imdb\.com/title/tt(\d+)', imdb_response)
        ...
        # Ask for category
        await message.reply_text("📂 Choose the category (e.g., Hollywood, Bollywood):")
        category = (await client.listen(message.chat.id)).text.strip()
        
        # Ask for quality
        await message.reply_text("📊 What is the quality of the movie? (e.g., CAM, HDrip, HDTV):")
        ...
        loading_sticker = await message.reply_sticker("⏳")  # Replace with your loading sticker ID
